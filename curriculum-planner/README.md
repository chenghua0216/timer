...
- **母版更新**：之後若 `Template.gs` 有新版，貼上後在編輯器執行一次 `refreshTemplate()`，
  會以新內容重建母版並自動封存舊版。
- **前端即時模式（預設開啟）**：`doGet()` 每次載入直接抓 GitHub 上最新的 `index.html` 顯示，
  因此**前端改版只要推上 GitHub 即自動生效**，不必貼檔或重新部署（約 90 秒快取，最長數分鐘內生效；
  想立即生效可在編輯器執行 `clearHtmlCache()`）。抓取失敗會自動退回專案內建的 index。
  關閉此模式：指令碼屬性設定 `LIVE_HTML = off`。首次啟用需貼一次新版 `Code.gs` 並重新部署。
- **一鍵更新（selfUpdate）**：在編輯器執行 `selfUpdate()`，自動從本 GitHub repo 抓最新
  程式碼、更新專案、母版有變才重建、建立新版本並把網頁部署切過去——之後改版不用再手動貼檔。
  一次性前置：到 [script.google.com/home/usersettings](https://script.google.com/home/usersettings)
  開啟「Google Apps Script API」，且 `appsscript.json` 需含 `script.projects`／
  `script.deployments`／`script.external_request` 三個權限（本檔已含）。
  抓取分支預設為開發分支，可用指令碼屬性 `SOURCE_BRANCH` 覆蓋（例如 `main`）；
  repo 若改為私有，再加屬性 `GITHUB_TOKEN`（fine-grained PAT，Contents: Read）。
...
> **檔案分享**：網頁以部署者身分執行，產出的簡報由部署者擁有。程式會自動把
> 每份產出與整個「心創力課程規劃簡報」資料夾設為**同網域（theshiner.org）可編輯**，
> 夥伴不會被要求「要求存取權」。若有更早、夥伴打不開的舊產出，在編輯器執行一次
> `shareFolder()` 即可一併補上分享權限。
